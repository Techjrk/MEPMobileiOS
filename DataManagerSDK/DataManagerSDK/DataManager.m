...
- (void)bidsRecentlyUpdated:(NSInteger)numberOfDays success:(APIBlock)success failure:(APIBlock)failure {
    ...
    NSString *filter = [NSString stringWithFormat:@"{\"include\":[\"projectStage\", {\"primaryProjectType\":{\"projectCategory\":\"projectGroup\"}}], \"where\":{\"lastPublishDate\":{\"gte\":\"%@\"}}, \"limit\":250,\"dashboardTypes\":true,\"order\":\"lastPublishDate DESC\"}", [DerivedNSManagedObject dateStringFromDateDay:previousMonth]];
...
- (void)projectsNear:(CGFloat)lat lng:(CGFloat)lng distance:(NSNumber*)distance filter:(id)filter success:(APIBlock)success failure:(APIBlock)failure {
    ...
    NSString *allFilter = @"{\"include\":[\"workTypes\",\"userNotes\",\"images\",\"projectStage\",{\"primaryProjectType\":{\"projectCategory\":\"projectGroup\"}},{\"contacts\":[\"company\"]}],\"limit\":200, \"order\":\"id DESC\",\"where\":{\"hidden\":false}}";
...
- (void)addProjectUserImage:(NSNumber*)projectID title:(NSString*)title text:(NSString*)text address:(NSString *)address image:(UIImage*)image geocode:(NSDictionary *)geocode success:(APIBlock)success failure:(APIBlock)failure {
    ...
    [self HTTP_POST_BODY:[self url:url] parameters:@{@"title":title, @"text":text, @"file":encodedImage,@"fullAddress":address,@"geocode":geocode} success:^(id object) {
        success(object);
    } failure:^(id object) {
        failure(object);
    } authenticated:YES];
...
- (void)updateProjectUserImage:(NSNumber *)projectID title:(NSString *)title text:(NSString *)text address:(NSString *)address image:(UIImage *)image geocode:(NSDictionary *)geocode success:(APIBlock)success failure:(APIBlock)failure{
    ...
    */
    ...
    [self HTTP_PUT_BODY:[self url:url] parameters:@{@"title":title, @"text":text, @"file":encodedImage,@"fullAddress":address,@"geocode":geocode} success:^(id object){
        success(object);
    }failure:^(id object){
        failure(object);
    }authenticated:YES];}
