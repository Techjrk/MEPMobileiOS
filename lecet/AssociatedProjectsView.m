...
@interface AssociatedProjectsView()<UICollectionViewDelegate, UICollectionViewDataSource, ActionViewDelegate, CustomCollectionViewDelegate, PopupViewControllerDelegate, TrackingListViewDelegate, NewTrackingListCollectionViewCellDelegate>{
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSDictionary *dict = @{ASSOCIATED_BID_ID:project.recordId, ASSOCIATED_BID_NAME:project.title, ASSOCIATED_BID_LOCATION:[project address], ASSOCIATED_BID_DESIGNATION:project.unionDesignation != nil?project.unionDesignation:@"", ASSOCIATED_BID_GEOCODE_LAT:project.geocodeLat, ASSOCIATED_BID_GEOCODE_LNG:project.geocodeLng, ASSOCIATED_BID_HAS_NOTESIMAGES:project.hasNotesImages};
    [cell setInfo:dict];
...
- (void)didTrackItem:(id)sender {
    ...
    [[DataManager sharedManager] projectAvailableTrackingList:projectId success:^(id object) {
        ...
        if (trackItemRecord.count>0) {
            ...
            CGRect rect = [controller getViewPositionFromViewController:[cell.actionView trackButton] controller:[self.associatedProjectDelegate parentController]];
            ...
            [[self.associatedProjectDelegate parentController] presentViewController:controller animated:NO completion:nil];
        } else {
...
- (void)didShareItem:(id)sender {
    ...
    PopupViewController *controller = [PopupViewController new];
    CGRect rect = [controller getViewPositionFromViewController:[cell.actionView shareButton] controller:[self.associatedProjectDelegate parentController]];
    controller.popupRect = rect;
    controller.popupWidth = 0.98;
    controller.isGreyedBackground = YES;
    controller.customCollectionViewDelegate = self;
...
- (void)getNewTrackingList {
    ...
    [[self.associatedProjectDelegate parentController] presentViewController:alert animated:YES completion:nil];
    
    
}

-(void)didTappedNewTrackingList:(id)sender {
    [[self.associatedProjectDelegate parentController] dismissViewControllerAnimated:NO completion:^{
        [self PopupViewControllerDismissed];
